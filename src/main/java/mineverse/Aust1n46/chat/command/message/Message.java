      ...
      if (args[1].length() > 0) {
        ...
        if (plugin.getConfig().getString("tellformatto").equalsIgnoreCase("Default")) {
          ...
          echo = Format.FormatStringAll(
              plugin.getConfig().getString("tellformatto").replace("{playerto}", player.getNickname()).replace("{playerfrom}", mcp.getNickname()))
              + msg;
        }
        if (plugin.getConfig().getString("tellformatfrom").equalsIgnoreCase("Default")) {
          ...
          send = Format.FormatStringAll(
              plugin.getConfig().getString("tellformatfrom").replace("{playerto}", player.getNickname()).replace("{playerfrom}", mcp.getNickname()))
              + msg;
        }
        if (plugin.getConfig().getString("tellformatspy").equalsIgnoreCase("Default")) {
          ...
          spy = Format.FormatStringAll(
              plugin.getConfig().getString("tellformatspy").replace("{playerto}", player.getName()).replace("{playerfrom}", mcp.getName())) + msg;
        }
