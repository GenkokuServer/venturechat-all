      ...
      if (args[0].length() > 1) {
        ...
        if (channel.hasPermission()) {
          ...
          if (!player.getPlayer().hasPermission(channel.getPermission())) {
            ...
            sender.sendMessage(ChatColor.RED + "This player does not have permission for channel: "
        ...
        sender.sendMessage(ChatColor.GOLD + "Set player " + ChatColor.RED + player.getName() + ChatColor.GOLD + " into channel: "
            + ChatColor.valueOf(channel.getColor().toUpperCase()) + channel.getName());
        if (player.hasConversation()) {
          for (MineverseChatPlayer p : MineverseChat.onlinePlayers) {
            if (p.isSpy()) {
              p.getPlayer().sendMessage(player.getName() + " is no longer in a private conversation with "
                  + MineverseChatAPI.getMineverseChatPlayer(player.getConversation()).getName() + ".");
            }
          }
          if (player.isOnline())
            player.getPlayer().sendMessage("You are no longer in private conversation with "
                + MineverseChatAPI.getMineverseChatPlayer(player.getConversation()).getName() + ".");
          else
