  ...
  public void execute(CommandSender sender, String command, String[] args) {
    ...
    if (mcp.getPlayer().hasPermission("venturechat.gui")) {
      ...
      if (target == null) {
        ...
        target = new MineverseChatPlayer(uuid, name, current, ignores, listening, mutes, blockedCommands, mail, false, null, true, true, name,
  ...
  private void openInventory(MineverseChatPlayer mcp, MineverseChatPlayer target, ChatChannel channel, int hash) {
    ...
    for (GuiSlot g : MineverseChat.gsInfo.getGuiSlots()) {
      if (!g.hasPermission() || mcp.getPlayer().hasPermission(g.getPermission())) {
        ...
        String displayName = g.getText().replace("{player_name}", target.getName()).replace("{channel}", channel.getName()).replace("{hash}",
            hash + "");
        if (target.isOnline()) {
