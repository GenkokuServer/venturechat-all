      ...
      if (args.length > 1) {
        ...
        if (match || player.isBlockedCommand(args[1])) {
          if (!player.isBlockedCommand(args[1])) {
            ...
            sender.sendMessage(
                ChatColor.RED + "Blocked player " + ChatColor.GOLD + player.getName() + ChatColor.RED + " from entering command " + args[1] + ".");
            return;
          }
          ...
          sender.sendMessage(
              ChatColor.RED + "Unblocked player " + ChatColor.GOLD + player.getName() + ChatColor.RED + " from entering command " + args[1] + ".");
          return;
        }
