...
public class Removemessage extends MineverseCommand {
      ...
      public void run() {
        ...
        for (MineverseChatPlayer p : MineverseChat.onlinePlayers) {
          ...
          for (ChatMessage message : messages) {
             ...
             * String submessage = message.getMessage().substring(0, message.getMessage().length() -
             * plugin.getConfig().getString("messageremovericon").length() - 1).replaceAll("(�([a-z0-9]))", "");
             * if(submessage.hashCode() == hash) {
             * WrappedChatComponent removedComponent = p.getPlayer().hasPermission("venturechat.message.bypass") ?
             * Removemessage.this.getMessageDeletedChatComponentAdmin(message) : Removemessage.this.getMessageDeletedChatComponentPlayer();
             * message.setComponent(removedComponent);
             * message.setHash(-1);
             * playerPackets.add(Removemessage.this.createPacketPlayOutChat(removedComponent));
             * resend = true;
  ...
  public WrappedChatComponent getMessageDeletedChatComponentAdmin(ChatMessage message) {
    ...
    if (message.getMessage().contains(ChatColor.stripColor(Format.FormatStringAll(plugin.getConfig().getString("guiicon"))))) {
      oMessage = oMessage.substring(0,
          oMessage.length() - ChatColor.stripColor(Format.FormatStringAll(plugin.getConfig().getString("guiicon"))).length() - 3) + "\"}]";
    }
    return WrappedChatComponent.fromJson(Format.FormatStringAll(
        "{\"text\":\"\",\"extra\":[{\"text\":\"&c&o<message removed>\",\"hoverEvent\":{\"action\":\"show_text\",\"value\":{\"text\":\"&7Message: \","
            + oMessage + "}}}]}"));
  }
}
