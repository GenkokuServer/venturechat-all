...
public class Kickchannel extends MineverseCommand {
  ...
  public void execute(CommandSender sender, String command, String[] args) {
    if (sender.hasPermission("venturechat.kickchannel")) {
      ...
      sender.sendMessage(ChatColor.GOLD + "Kicked player " + ChatColor.RED + args[0] + ChatColor.GOLD + " from channel: "
      ...
      if (player.getListening().size() == 0) {
        ...
        if (player.isOnline()) {
          ...
          player.getPlayer()
              .sendMessage("Channel Set: " + ChatColor.valueOf(cc.defaultColor.toUpperCase()) + "[" + cc.getDefaultChannel().getName() + "]");
        } else
          player.setModified(true);
      }
      return;
