...
public class PacketListener extends PacketAdapter {
  ...
  public void onPacketSending(PacketEvent event) {
    if (event.isCancelled() || event.getPacketType() != PacketType.Play.Server.CHAT) { return; }
    ...
    try {
      if (VersionHandler.is1_7_10() || VersionHandler.is1_7_9() || VersionHandler.is1_7_2()) {
        if ((MineverseChat.posField != null) && !(((boolean) MineverseChat.posField.get(event.getPacket().getHandle())))) { return; }
      } else if (VersionHandler.is1_8()) {
        if ((MineverseChat.posField != null) && (((Byte) MineverseChat.posField.get(event.getPacket().getHandle())).intValue() > 1)) { return; }
      } else if (VersionHandler.is1_9() || VersionHandler.is1_10() || VersionHandler.is1_11()) {
        if ((MineverseChat.posField != null) && (((Byte) MineverseChat.posField.get(event.getPacket().getHandle())).intValue() > 1)) { return; }
      } else {
        if ((MineverseChat.posField != null) && ((Object) MineverseChat.posField.get(event.getPacket().getHandle())) == MineverseChat.chatMessageType
            .getEnumConstants()[2]) { return; }
      }
    ...
    if (lastChatMessage != null && lastChatMessage.getHash() == hash) {
       ...
       * json += ",{\"text\":\" " + Format.FormatStringAll(plugin.getConfig().getString("messageremovericon")) +
       * "\",\"clickEvent\":{\"action\":\"run_command\",\"value\":\"/removemessage " + lastChatMessage.getHash() + " true"
       * +"\"},\"hoverEvent\":{\"action\":\"show_text\",\"value\":{\"text\":\"" +
       * Format.FormatStringAll(plugin.getConfig().getString("messageremovertext")) + "\"}}}]";
       * }
       * if(!mcp.getButtons()) {
       * for(JsonButton j : MineverseChat.jbInfo.getJsonButtons()) {
       * if(j.hasPermission() && mcp.getPlayer().hasPermission(j.getPermission())) {
       * json = json.substring(0, json.length() - 1);
       * json += ",{\"text\":\" " + Format.FormatStringAll(j.getIcon()) + "\",\"clickEvent\":{\"action\":\"" + j.getAction() + "\",\"value\":\"/" +
       * j.getCommand().replace("{channel}", lastChatMessage.getChannel()).replace("{player}", lastChatMessage.getSender()) +
       * "\"},\"hoverEvent\":{\"action\":\"show_text\",\"value\":{\"text\":\"" + Format.FormatStringAll(j.getText()) + "\"}}}]";
       * }
       * }
       * }
       */
      if (mcp.getPlayer().hasPermission("venturechat.gui")) {
        ...
        json += "," + Format.convertToJsonColors(Format.FormatStringAll(plugin.getConfig().getString("guiicon")),
            ",\"clickEvent\":{\"action\":\"run_command\",\"value\":\"/vchatgui " + lastChatMessage.getSender() + " " + lastChatMessage.getChannel()
                + " " + lastChatMessage.getHash() + "\"},\"hoverEvent\":{\"action\":\"show_text\",\"value\":{\"text\":\"\",\"extra\":["
                + Format.convertToJsonColors(Format.FormatStringAll(plugin.getConfig().getString("guitext"))) + "]}}")
            + "]}]";
      }
