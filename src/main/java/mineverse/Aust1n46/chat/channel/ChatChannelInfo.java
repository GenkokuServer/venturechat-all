...
public class ChatChannelInfo {
  ...
  public ChatChannelInfo(MineverseChat _plugin) { // Contructor that creates the channels and puts them into their array
    ...
    for (String key : cs.getKeys(false)) {
      ...
      ChatChannel c = new ChatChannel(_name, _color, _chatcolor, _permission, _mutable, _filter, _defaultchannel, _alias, _distance, _autojoin,
          _bungee, _cooldown, _format, irc);
      cc[x++] = c;
      if (_defaultchannel) {
        defaultChannel = c;
        defaultColor = _color;
  ...
  void logChannelList() {
    for (ChatChannel p : cc) {
      plugin.getServer().getLogger().info("[" + plugin.getName() + "]" + p.getName() + ":" + p.getChatColor() + ":" + p.getPermission() + ":"
          + p.isMutable() + ":" + p.isFiltered() + ":" + p.isDefaultchannel());
    }
  }
  ...
  public ChatChannel getChannelInfo(String ChannelName) {
    for (ChatChannel c : cc) {
      if (c.getName().equalsIgnoreCase(ChannelName) || c.getAlias().equalsIgnoreCase(ChannelName)) { return c; }
    }
    return null;
  }
