...
public class MineverseChat extends JavaPlugin implements PluginMessageListener {
  ...
  public void onEnable() {
    ...
    if (this.firstRun) {
      for (String uuidString : PlayerData.getPlayerData().getConfigurationSection("players").getKeys(false)) {
        ...
        StringTokenizer b = new StringTokenizer(
            PlayerData.getPlayerData().getConfigurationSection("players." + uuidString).getString("blockedcommands"), ",");
        ...
        UUID party = PlayerData.getPlayerData().getConfigurationSection("players." + uuidString).getString("party").length() > 0
            ? UUID.fromString(PlayerData.getPlayerData().getConfigurationSection("players." + uuidString).getString("party"))
            : null;
        ...
        players.add(new MineverseChatPlayer(uuid, name, currentChannel, ignores, listening, mutes, blockedCommands, mail, host, party, filter,
            notifications, nickname, jsonFormat, spy, commandSpy, rangedSpy, buttons, messageToggle, bungeeToggle));
      }
    } else {
    ...
    if (this.getConfig().getConfigurationSection("mysql").getBoolean("enabled")) {
      this.MySQL = new MySQL(this, getConfig().getConfigurationSection("mysql").getString("host"),
          getConfig().getConfigurationSection("mysql").getString("port"), getConfig().getConfigurationSection("mysql").getString("database"),
          getConfig().getConfigurationSection("mysql").getString("user"), getConfig().getConfigurationSection("mysql").getString("password"));
      this.mysql = true;
      try {
        c = MySQL.openConnection();
        Statement statement = c.createStatement();
        statement.executeUpdate(
            "CREATE TABLE IF NOT EXISTS `VentureChat` (`rowid` INT(7) NOT NULL AUTO_INCREMENT, `ChatTime` TEXT(100), `UUID` TEXT(100), `Name` TEXT(100), `Server` TEXT(100), `Channel` TEXT(100), `Text` TEXT(300), `Type` TEXT(100), PRIMARY KEY (rowid));");
          ...
          for (String c : p.getMutes().keySet()) {
            ...
            if (time > timemark) {
              ...
              if (p.isOnline()) p.getPlayer().sendMessage(
                  ChatColor.RED + "You have just been unmuted in: " + ChatColor.valueOf(channel.getColor().toUpperCase()) + channel.getName());
  ...
  public void onPluginMessageReceived(String channel, Player player, byte[] message) {
    if (!channel.equals("venturechat:")) { return; }
    try {
      ...
      if (subchannel.equals("Message")) {
        ...
        if (identifier.equals("Send")) {
          ...
          if (s != null) {
            ...
            s = new MineverseChatPlayer(uuid, name, current, ignores, listening, mutes, blockedCommands, mail, false, null, true, true, name,
                jsonFormat, false, false, false, true, true, true);
      ...
      if (subchannel.equals("Mute")) {
        ...
        if (!time.equals("None\n")) {
          try {
            ...
            if (numtime > 0) {
              ...
              if (p.isOnline()) p.getPlayer().sendMessage(ChatColor.RED + "You have just been muted in: "
                  + ChatColor.valueOf(cc.getColor().toUpperCase()) + cc.getName() + ChatColor.RED + " for " + time + " " + keyword);
              else p.setModified(true);
              if (cc.getBungee()) {
        ...
        if (p.isOnline())
          p.getPlayer().sendMessage(ChatColor.RED + "You have just been muted in: " + ChatColor.valueOf(cc.getColor().toUpperCase()) + cc.getName());
        else p.setModified(true);
        if (cc.getBungee()) {
          MineverseChat.getInstance().synchronize(p, true);
        }
      ...
      if (subchannel.equals("Unmute")) {
        ...
        if (p.isOnline()) p.getPlayer()
            .sendMessage(ChatColor.RED + "You have just been unmuted in: " + ChatColor.valueOf(cc.getColor().toUpperCase()) + cc.getName());
        else p.setModified(true);
        if (cc.getBungee()) {
          MineverseChat.getInstance().synchronize(p, true);
        }
