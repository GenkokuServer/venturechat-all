...
public final class Format {
  ...
  public static String convertToJson(ChatMessage lastChatMessage) {
    ...
    try {
      ...
      if (plugin.getConfig().getString("loglevel", "info").equals("debug")) {
        Bukkit.getConsoleSender()
            .sendMessage(Format.FormatStringAll("&8[&eVentureChat&8]&e - Prefix and / or suffix don't exist, setting to nothing."));
      }
      suffix = "venturechat_no_suffix_code";
      prefix = "venturechat_no_prefix_code";
    }
    ...
    // json += "]},{\"text\":\"" + lowerText + "\",\"clickEvent\":{\"action\":\"" + clickLowerAction + "\",\"value\":\"" + clickLowerActionText +
    // "\"},\"hoverEvent\":{\"action\":\"show_text\",\"value\":{\"text\":\"\",\"extra\":[" + convertToJsonColors(hoverLower) + "]}}}";
    // json += ",{\"text\":\"\",\"extra\":[" + convertToJsonColors(getLastCode(lowerText) + f.substring(lower + lowerText.length(), middle)) + "]}";
    // json += ",{\"text\":\"" + getLastCode(lowerText + f.substring(lower + lowerText.length(), middle)) + middleText +
    // "\",\"clickEvent\":{\"action\":\"" + clickMiddleAction + "\",\"value\":\"" + clickMiddleActionText +
    // "\"},\"hoverEvent\":{\"action\":\"show_text\",\"value\":{\"text\":\"\",\"extra\":[" + convertToJsonColors(hoverMiddle) + "]}}}";
    // json += "," + convertToJsonColors(getLastCode(lowerText + f.substring(lower + lowerText.length(), middle)) + f.substring(middle +
    // middleText.length(), f.length())) + "," + convertLinks(getLastCode(lowerText + f.substring(lower + lowerText.length(), middle) +
    // f.substring(middle + middleText.length(), f.length())) + c);
  ...
  private static String convertPlaceholders(String s, JsonFormat format, String prefix, String nickname, String suffix, MineverseChatPlayer icp) {
    ...
    do {
      Pattern pattern = Pattern.compile(
          "(" + prefix.replace("[", "\\[").replace("]", "\\]").replace("{", "\\{").replace("}", "\\}").replace("(", "\\(").replace(")", "\\)") + "|"
              + nickname.replace("[", "\\[").replace("]", "\\]").replace("{", "\\{").replace("}", "\\}").replace("(", "\\(").replace(")", "\\)") + "|"
              + suffix.replace("[", "\\[").replace("]", "\\]").replace("{", "\\{").replace("}", "\\}").replace("(", "\\(").replace(")", "\\)") + ")");
      ...
      if (matcher.find()) {
        ...
        temp += convertToJsonColors(lastCode + placeholder, ",\"clickEvent\":{\"action\":\"" + action + "\",\"value\":\"" + text
  ...
  private static final Pattern LINKS_PATTERN = Pattern.compile("([a-zA-Z0-9�\\-:/]+\\.[a-zA-Z/0-9�\\-:_#]+(\\.[a-zA-Z/0-9.�\\-:#\\?\\+=_]+)?)");
    ...
    do {
      ...
      if (matcher.find()) {
        ...
        temp += convertToJsonColors(lastCode + link,
            ",\"underlined\":\"" + plugin.getConfig().getBoolean("underlineurls", true)
  ...
  public static String getLastCode(String s) {
    ...
    final StringBuilder buffer = new StringBuilder(ch.length);
    for (int i = 0; i < ch.length - 1; ++i) {
      buffer.append(ch[i]).append(ch[i + 1]);
      if ("lkonmr0123456789abcdef".indexOf(ch[i + 1]) != -1 && ch[i] == '�') {
        if ("0123456789abcdefr".indexOf(ch[i + 1]) != -1) {
          buffer.setLength(0);
          buffer.append(ch[i]).append(ch[i + 1]);
        }
      }
  ...
  public static String convertToJsonColors(String s, String extensions) {
    ...
    do {
      ...
      temp += "{\"text\":\"" + remaining.substring(0, indexNextColor) + "\",\"color\":\"" + hexidecimalToJsonColor(color) + "\"" + modifier
          + extensions + "},";
  ...
  protected static final Pattern chatColorPattern = Pattern.compile("(?i)&([0-9A-F])");
  protected static final Pattern chatMagicPattern = Pattern.compile("(?i)&([K])");
  protected static final Pattern chatBoldPattern = Pattern.compile("(?i)&([L])");
  protected static final Pattern chatStrikethroughPattern = Pattern.compile("(?i)&([M])");
  protected static final Pattern chatUnderlinePattern = Pattern.compile("(?i)&([N])");
  protected static final Pattern chatItalicPattern = Pattern.compile("(?i)&([O])");
  protected static final Pattern chatResetPattern = Pattern.compile("(?i)&([R])");
