...
public class MessageListener extends ListenerAdapter {
  ...
  public void onMessage(MessageEvent event) {
    ...
    plugin.getServer().getConsoleSender()
        .sendMessage(ChatColor.RED + "<IRC> " + user.getNick() + ": " + ChatColor.WHITE + Format.FormatStringAll(event.getMessage()));
  }

  public boolean hasPermission(User user, String mode, Channel channel) {
    switch (mode) {
      case "v": {
        return user.getChannelsVoiceIn().contains(channel) || user.getChannelsOpIn().contains(channel) || user.getChannelsOwnerIn().contains(channel)
            || user.getChannelsHalfOpIn().contains(channel);
      }
      case "o": {
      ...
      case "h": {
        return user.getChannelsHalfOpIn().contains(channel) || user.getChannelsOpIn().contains(channel)
            || user.getChannelsOwnerIn().contains(channel);
      }
    }
    return false;
  }
