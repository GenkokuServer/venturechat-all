...
public class VentureChatPlaceholders extends PlaceholderHook {
  ...
  public String onPlaceholderRequest(Player p, String identifier) {
    ...
    if (mcp == null) { return ""; }
    if (identifier.equalsIgnoreCase("nickname")) { return mcp.hasNickname() ? mcp.getNickname() : mcp.getName(); }
    if (identifier.equalsIgnoreCase("something_else_you_think_of")) { return "value for that identifier *"; }
    if (identifier.startsWith("channel_")) {
      if (mcp.getCurrentChannel() == null) { return ""; }
      switch (identifier) {
        case "channel_name":
